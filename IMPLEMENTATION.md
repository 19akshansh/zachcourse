...
- **Defensive Element Skipping:** If expected selector nodes are missing or cannot be located, the controller implements automatic recovery attempts (such as loading database queries or sidebars) and falls back to skipping the missing step smoothly instead of freezing the onboarding experience.

### Strict Resource Link Safety & Sanitization
- **Verifiable URL References:** Lesson learning resources generated by the Visual Roadmap Agent are subjected to prompt-level rules that forbid imaginary domain deep-links and restrict generated entries to trustworthy developer destinations (e.g., MDN, react.dev, Python Docs, GitHub, YouTube).
- **Dual-Layer Google Search Fallback:** For maximum visual stability, both the server-side API (during roadmap generation) and the client-side renderer (`VisualRoadmapGraph.tsx`) run URLs through `sanitizeResourceUrl` (defined in `src/lib/resource-link.ts`). Any malformed url or hostname matching a blocklist of dummy placeholder domains (e.g., `example.com`, `placeholder.com`) is immediately translated into a guaranteed-real, topic-scoped Google Search query URL (`https://www.google.com/search?q=...`), keeping links interactive, informative, and safe.
